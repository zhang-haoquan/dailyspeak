...
>
> **已改成「启动即报错退出」**，避免 `shots.mjs` 静默产出 14 张错误截图。
> **重写时机：P2 结束后一次性改对**——现在改等于做两遍（`today`/`history` 接口在 S3 才有，
> 学习链路与评分要等 P2）。**不要删这两个文件**，里面 73 项断言编码的是 PRD 业务规则，
> 是重写时的规格参照。这期间回归靠 `npm run smoke` + `tools/auth-ui.mjs`，
> 且 S3 每新增一个接口都会配套补测试。
...
- [ ] `tools/shots.mjs` 同样重写（先登录再截图），恢复 14 张页面截图能力
