...
| AI | LLM：DeepSeek（`deepseek-flash`）｜ ASR：腾讯云一句话识别（`16k_en`） |
| 首发领域 | 计算机 / IT + 职场通用 |
...
**版本标注**：相对 v0.1 的新增/修订以 `【v0.2】` 标注，相对 v0.2 的新增/修订以 `【v0.3】` 标注。
...
| v0.3 | 2026-09-11 | 5.3 补齐排卡口径（复习不截断、未选领域的行为）；**新增 5.6 学习记录页统计口径**；**新增 12 章接口契约**；8 章补充统一错误格式、服务器时区与前端技术选型；9 章补充内容不足的告警与补齐顺序 | 开发过程决策 [D-027 ~ D-031](../DECISIONS.md) |
| v0.4 | 2026-09-11 | **6 章补齐评分链路的落地口径**（音频格式、重试次数、降级语义、实测时延）；**5.4 明确「录音结束即提交评测」**；9 章明确音频格式约束与「识别不到内容」的处理；**12 章补齐两个评分接口的契约**（含 `score` 可空与 `degraded`） | 开发过程决策 [D-036 ~ D-041](../DECISIONS.md) |
...
- **复习卡不截断**：到期复习卡**全部**进入今日列表，再多的到期卡也不削减。
  新句名额 = `max(0, 每日条数 − 今日到期复习卡数)`。因此在复习积压时，
  今日总条数会**超过**每日条数——这是刻意的：宁可一天多做几张，
  也不能让到期卡继续往后拖，否则遗忘曲线的间隔就不成立了（PRD 5.5）。
- **未选领域时不对领域设限**：用户尚未完成首次引导（`domains` 为空）时，
  所有已上线内容都可作为新句推送，且**不算**「跨领域补齐」，不产生告警。
- **补齐顺序**（PRD 09）固定为：① 只完成跟读的卡（按完成时间升序）→
  ② 已选领域内没学过的卡（随机）→ ③ 其他有内容的领域（首发领域优先，其余按字典序，随机）。
- **随机用无偏洗牌**（Fisher-Yates）。形如 `sort(() => Math.random() - 0.5)` 的
  「洗牌」分布是偏的，会让靠前的卡长期更容易被推到，属于已修复的缺陷。
- **快照生成是幂等的**：同一用户同一天并发首次进入，唯一键 `(user_id, date_key)`
  保证只会落库一份，后到的请求读取已生成的那份。
...
- 重新录音时回收上一段音频资源。
- 音频是否持久化到服务端见 [D-017](../DECISIONS.md#d-017-音频不落库已定)。

> 决策依据：D-007

### 【v0.4】跟读的提交流程：录音一结束就提交

录音停止后**立即**上传评测，而不是等用户点「完成」：

- 理由一：PRD 5.4 要求「录音结束后」就能看到「我听到的」，转写来自服务端 ASR，不提交就拿不到。
- 理由二：状态流转表里「待跟读 → 跟读已打分」的触发条件本来就是「录音提交并返回分」。
- 「完成」按钮因此不再触发评测，只负责跳转到结果页；它在上传成功前**保持禁用**，避免用户在没有分数的情况下进入下一页。
- 评测失败（含时长不合规）时按钮保持禁用并显示原因，用户可以直接重录。

**录音时长自动封顶**：跟读到 14.5 秒、应答到 59 秒自动停止，避免用户录过头后被服务端直接拒绝（窗口见 09 章）。自动停止时界面会说明原因。
...
展示三块：顶部三个数字（连续学习天数 / 本月学习卡片 / 累计学习卡片）、本周学习日历图、最近练习列表。

#### 【v0.3】统计口径

| 指标 | 口径 |
| --- | --- |
| 连续学习天数 | 有任意一次有效提交即算「这天学过」。**从今天往前连续计数；若今天还没学，则从昨天往前数**（今天、昨天都没有记录才为 0）。 |
| 本月学习卡片 | `user_progress` 中 `done_at` 落在**本地日期**本月范围内的记录数 |
| 累计学习卡片 | `user_progress` 记录总数（每张卡一条，重复练习不重复计数） |
| 本周趋势 | 固定 7 个桶，**周一 → 周日**，越界记录不计入 |
| 最近练习 | 按 `done_at` 倒序取最近 8 条 |

**「今天还没学就从昨天数」的理由**：用户早上打开应用时今天必然还没学，
若直接从今天数，连续 6 天的记录会在每天早上显示成「连续 0 天」，
把昨天为止的成绩抹掉——这是很常见的体验事故。

**打分展示口径**：最近练习只展示**综合分**。只做了跟读没做应答（未完成），
或第三方评测失败降级（PRD 09）的记录，一律显示**「未打分」**，
**不得用跟读分、0 分或任何默认值顶替**。历史页要能区分「已完成」与「已完成、未打分」。

所有日期口径统一走**本地日期**，不得用 UTC 字符串前缀比较。
...
统一接入国内大模型服务商，并**做成可替换的供应商接口**。MVP 采用「ASR + 大模型语义评测」方案，发音精度通过转写一致性间接体现，后续如需音素级精修再接专业语音 SDK。

本项目已选定的供应商（【v0.3】）：

| 环节 | 供应商与型号 | 备注 |
| --- | --- | --- |
| 语音转文字 | **腾讯云一句话识别 `SentenceRecognition`**（`EngSerViceType = 16k_en`） | 同步返回，≤ 60 秒；见 [D-013](../DECISIONS.md#d-013-asr-供应商选定腾讯云语音识别) |
| 语义评测 | **DeepSeek `deepseek-flash`**（OpenAI 兼容接口） | 见 [D-012](../DECISIONS.md#d-012-llm-供应商选定-deepseek)、[D-039](../DECISIONS.md#d-039-deepseek-实际模型名与推理模型的-max_tokens-陷阱) |

### 两层评测模型

- **语义层**（大模型擅长）：把用户的语音转成文本后交给 LLM（DeepSeek），判断情境应答的「内容对不对、语法、措辞、完整性」。
...
**音频入口**：服务端**只接受 16kHz 单声道 16-bit PCM WAV**。

- 腾讯云一句话识别不支持 webm，而 Chrome 的 `MediaRecorder` 默认录的就是 webm → **浏览器端先转码**（`decodeAudioData` → 降混单声道 → 重采样到 16k → 编码 WAV）。
- 重采样优先交给 `OfflineAudioContext`，浏览器自带的重采样质量好于手写插值。
- 服务端**自己解析 WAV 头**得出时长与采样率，**不采信客户端上报的 `durationMs`**——时长窗口是准入条件，不能由客户端说了算。WAV 是分块格式，解析按块遍历，不按固定偏移硬读。

**调用策略**：

| 项 | 口径 |
| --- | --- |
| 重试 | 第三方超时/失败**最多尝试 3 次**（首次 + 2 次重试），指数退避 300ms → 1200ms |
| 识别为空 | 服务端返回 **400**「没有识别到任何内容，请靠近麦克风、大声朗读后重试」。**不伪造分数**塞进历史 |
| 重试耗尽 | 降级：只记完成、`score` 为 `null`、`degraded: true`，见 09 章 |
| 评测温度 | `temperature: 0`，让同一个回答尽量得到同一个分数 |
| 排期推进 | 只在**整卡完成**（应答打分）时推进一次，且只在排期**已到期**时推进（D-006 / D-038） |

**实测时延**（6.3 秒跟读 + 34 秒应答，本地环境）：

| 环节 | 耗时 |
| --- | --- |
| 腾讯云一句话识别（6.3 秒音频） | 约 620–740ms |
| DeepSeek 四维评测 | 约 2.9–3.8s |
| 跟读整体（ASR + 词级比对 + 落库） | 约 670ms |

**分数来源必须可复算**：跟读分 100% 由词级相似度决定（`repeatScoreFromSimilarity`），应答分是四维加权（40/25/20/15），综合分是两者各半。三处公式都在 `packages/shared`，前后端与展示共用一份，接口返回的分数必须能用返回的中间量复算出来——端到端测试就是这么验的。
...
| AI 评测 | ASR：腾讯云一句话识别 ｜ LLM：DeepSeek | 国内访问、合规与成本友好，接口可替换（D-012 / D-013） |
| 语音采集 | 浏览器 MediaRecorder / WebRTC | 前端直接采集录音，上传后端评测 |

登录设计（低成本 + 可扩展）：MVP 采用邮箱 + 密码（Supabase Auth 开箱即用，几乎零成本）；微信 OAuth、手机号短信作为后台开关，后期直接开启，无需重写认证逻辑（认证范围见 [D-015](../DECISIONS.md#d-015-认证范围只做邮箱--密码)）。
...
- **统一错误格式**：所有接口失败一律返回 `{ code, message, details? }`；
  `code` 是稳定标识（供前端分支判断），HTTP 状态码保留语义。
  `details` 用于字段级校验错误，键为字段名，供前端直接高亮输入框。
- **成功响应不加 envelope**：直接返回资源本身，不套 `{ data: ... }`。
- **不发散内部错误**：5xx 只回「服务内部错误」，堆栈与 SQL 细节只进服务端日志。
- **每请求一行日志**：方法、路径、状态码、耗时、用户 id。**不打印请求体与响应体**——
  里面会有录音 base64 与转写文本，日志会被打爆。
- **服务端时区必须与用户一致**：「今天」「本月」「连续天数」全部按**本地日期**判定。
  部署时须显式设置 `TZ=Asia/Shanghai`（见 [TODO H](../TODO.md#h-上线前检查清单准备部署到自有服务器时执行--d-020)），
  否则跨时区会算错日期。
- **前端数据获取用 TanStack Query**：统一处理加载态、错误态、重试与缓存，
  不在页面里手写 `useEffect` + `useState` 三件套。
- **前后端共用类型**：所有跨端形状定义在 `packages/shared`，前端直接消费其**源码**
  （见 [D-025](../DECISIONS.md#d-025-前端直接消费-packagesshared-源码后端消费其-cjs-产物)），不复制第二份。
...
| 异常 | 处理策略 |
| --- | --- |
| AI/第三方超时或失败 | 自动重试，**最多尝试 3 次**；仍失败则降级为「仅记已完成、不产生打分」，引导用户稍后重测 |
| 录音失败 / 麦克风权限拒绝 | 提示授权并重试；提供文本输入兜底，允许不录音直接看参考答案 |
| 网络离线 | 前端缓存今日卡数据，联网后补齐提交；复习排期在服务端兜底计算 |
| 内容某领域无卡可推 | 从其他有内容的领域补充，保证每日条数不中断；后台打印内容告警 |
...
**格式与大小**（服务端强校验，全部返回 400 与中文提示）：

| 项 | 取值 |
| --- | --- |
| 格式 | 16kHz 单声道 16-bit **未压缩 PCM WAV**（只认这一种） |
| 跟读时长 | 3–15 秒 |
| 应答时长 | 30–60 秒 |
| 单文件上限 | 5 MB（60 秒 16k 单声道实测约 1.9MB） |

超限一律**拒绝并提示实际值**（例如「跟读至少要说满 3 秒（本次 2.0 秒），请重录」），不截断、不打折。

**降级（第三方重试耗尽）**：

- `user_progress` 里**只更新完成时间，不写分数**，也**不覆盖已有分数**——上次拿到 88 分，不能因为这次第三方挂了就抹掉。
- 接口返回 `degraded: true` 且 `score: null`，**前端必须显示「未打分」**，不得显示 0 分或用跟读分顶替。
- 行为上按「**未完成、待重测**」处理：不进「已掌握」，下次排卡会重新推给用户；历史页如实显示为「未打分」。
- **不新增数据库字段**：用「行存在但分数为 null」表达降级即可，理由见 [D-037](../DECISIONS.md#d-037-第三方失败降级只记完成不打分不新增字段)。

**录音不落库**（[D-017](../DECISIONS.md#d-017-音频不落库已定)）：音频只在内存里流到 ASR，用完即弃。回听只限当次会话，刷新后消失；逐词反馈与转写同理，**只存分数**。

### 【v0.3】内容不足的补齐顺序与告警

补齐顺序固定，不依赖数据库返回顺序（同一份内容池结果可复现）：

1. **只完成跟读的卡**（按完成时间升序）——优先让用户把半成品做完；
2. **已选领域内没学过的卡**（无偏随机）；
3. **其他有内容的领域**（首发领域优先，其余按字典序，无偏随机）。

告警文案（`contentWarning`）分两种，前端原样展示：

| 情形 | 文案 |
| --- | --- |
| 凑不满新句名额 | 「内容库储备不足：今日新句 X/Y 条，我们正在补充更多场景卡」 |
| 用了跨领域补齐 | 「已选领域的卡片都已学过，今日新句从其他领域补充」 |

同时在**服务端日志**打 `WARN`，作为内容管道的补充信号。
内容充足时不产生任何告警（`contentWarning` 为 `null`）。
...
> 【v0.3】本章是前后端共同遵守的接口口径，类型定义落在 `packages/shared/src/api.ts`，
> 前端不得自行复制一份。

### 12.1 通用约定

- 前缀 `/api`；**默认需要登录**（`Authorization: Bearer <Supabase access_token>`），
  仅健康检查等少数接口用 `@Public()` 放行。
- **成功响应直接返回资源本体**，不套 `{ data: ... }`。
- **失败统一返回** `{ code, message, details? }`，HTTP 状态码保留语义（401/403/404/409/422/5xx）。
- `details` 只用于**字段级校验错误**，键是字段名，值是提示文案。
- 5xx 不回传内部细节；每请求一行日志（方法、路径、状态码、耗时、用户 id），
  **不记录请求体与响应体**（含录音 base64 与转写文本）。

### 12.2 接口清单

| 方法 | 路径 | 说明 | 落地阶段 |
| --- | --- | --- | --- |
| GET | `/api/health` | 健康检查（`@Public`）：进程存活 + 数据库连通 + 内容卡数量 | P1 S1 |
| GET | `/api/auth/me` | 当前登录用户 + 画像（前端启动用它决定去登录页/引导页/学习台） | P1 S2 |
| GET | `/api/profile` | 读取学习计划 | P1 S2 |
| PUT | `/api/profile` | 保存学习计划（领域 1–3、每日条数 1–10，越界按边界截断） | P1 S2 |
| GET | `/api/cards/:id` | 卡片详情：内容 + 我的进度 + 我的复习阶段 | **P1 S3** |
| GET | `/api/today` | 今日任务快照 | **P1 S3** |
| GET | `/api/history` | 学习记录统计 | **P1 S3** |
| POST | `/api/score/repeat` | 跟读评测（ASR + 词级比对） | **P2** |
| POST | `/api/score/answer` | 情境应答评测（DeepSeek 四维） | **P2** |

### 12.3 `GET /api/cards/:id`

一次返回「内容 + 我的进度 + 我的复习阶段」三块，避免学习页串行打三个接口。
...
```jsonc
{
  "card": {
    ...
    "prompt": "Could you describe a project you are most proud of...",
    "promptZh": "能介绍一下你最引以为豪的项目吗？",
    "referenceAnswer": "…",          // 没有参考答案时该字段不下发
    "difficulty": 1,
    "tag": "面试高频句"
  },
  "progress": {                       // 没学过 → null
    ...
    "answerScore": 82,
    "composite": 85,
    "doneAt": "2026-09-11T02:31:00.000Z",
    "mode": "new"
  },
  "review": {                         // 未入复习队列 → null
    "stage": 2,
...
**边界**：卡片不存在、或 `status ≠ published`，一律返回 **404 `NOT_FOUND`**
（不返回 403，避免被探测出草稿卡的存在）。**不下发 `status` / `createdAt` 等审核字段**。
...
```jsonc
{
  ...
  "cards": [
    {
      "card": { /* ScenarioCard，同上 */ },
      "type": "new",                  // new | review
      "repeatDone": false,            // 只看「今天这一轮」的提交
      "answerDone": false,
      "stage": 2,                     // 仅 review 卡有
      "lastScore": 85                 // 仅 review 卡有
    }
  ],
  "contentWarning": null              // 内容不足时为提示文案
}
```

**注意：这是 GET，但当天首次调用会生成并落库快照**——生成是幂等的
（唯一键 `(user_id, date_key)`），当天重复请求返回完全相同的顺序。
排卡规则见 5.3，补齐与告警见 9 章。

### 12.5 `GET /api/history`
...
```jsonc
{
  ...
  "monthCards": 42,
  "totalCards": 118,
  "weekActivity": [3, 2, 0, 4, 1, 0, 0],  // 周一→周日，固定 7 个
  "recent": [
    { "date": "2026-09-11", "cardId": "c01", "title": "Resume walkthrough", "score": 85 }
  ]
}
```

`score` 为 `null` 表示**未打分**（只做了跟读未完成应答，或第三方失败降级），
前端必须显示「未打分」而**不是 0 分**。口径见 5.6。

### 12.6 `POST /api/score/repeat` 与 `POST /api/score/answer`

**请求**：`multipart/form-data`

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `audio` | file | 16kHz 单声道 16-bit PCM WAV（约束见 09 章） |
| `cardId` | string | 卡片 id |

**响应**：跟读返回 `RepeatScore`，应答返回 `AnswerScore`。**状态码 200**（不是 201）：
这两个接口是幂等的计算，不新建带 URL 的资源，重复提交只会覆盖同一行。
...
{
  "degraded": false,
  "score": 98,                       // 40–98，完全由 similarity 复算得出
  "feedback": [
    { "ok": true,  "text": "整句每个词都识别正确，发音清晰" },
    { "ok": true,  "text": "重音与连读位置基本准确" }
  ],
  "transcript": "I am a software developer with three years of experience and I really enjoy solving problems.",
  "transcriptSource": "tencent-asr",
  "alignment": [ { "word": "i", "said": "i", "ok": true } ],   // 逐词对照原句
  "similarity": 1
}
```
...
```jsonc
{
  "degraded": false,
  "score": 88,
  "dimensions": { "content": 88, "grammar": 86, "fluency": 89, "vocabulary": 88 },
  "feedback": [ { "ok": true, "text": "回答切题完整，观点清晰" } ],
  "suggestion": "内容具体、数据清晰；注意控制在 30 秒内。",
  "transcript": "…",
  "transcriptSource": "tencent-asr"
}
```

**降级**（第三方重试耗尽，此时 `score` 为 `null`）：

```jsonc
{
  "degraded": true,
  "score": null,
  "degradedReason": "语音评测服务暂时不可用，本次已记录完成但未打分，稍后可重测",
  "feedback": [ { "ok": false, "text": "本次没有产生打分，稍后重测即可" } ]
}
```

**`score` 是可空的**，前端必须显式处理 `null`（显示「未打分」），不得当作 0 分。

**失败情形**（均返回 400 + 中文提示，可安全展示给用户）：

| 情形 | 提示 |
| --- | --- |
| 没有 `audio` 字段 | 请上传音频文件（字段名 audio） |
| 不是 WAV / 不是未压缩 PCM | 音频格式不受支持，请重新录制 |
| 采样率或声道数不符 | 音频采样率应为 16000Hz，实际是 44100Hz，请重新录制 |
| 时长不在窗口内 | 跟读至少要说满 3 秒（本次 2.0 秒），请重录 |
| 时长上限 | 跟读不能超过 15 秒（本次 16.4 秒），请重录 |
| 音频过大 | 音频文件过大，请重新录制 |
| 没识别到内容 | 没有识别到任何内容，请靠近麦克风、大声朗读后重试 |
| 卡片不存在 / 未上线 | 卡片不存在或尚未上线（404） |

### 12.7 错误码

| code | 典型 HTTP | 含义 |
| --- | --- | --- |
| `BAD_REQUEST` | 400 | 请求本身不合法（含 Prisma 外键约束失败） |
| `UNAUTHORIZED` | 401 | 缺少令牌、令牌无效或已过期 |
| `FORBIDDEN` | 403 | 已登录但无权访问该资源 |
| `NOT_FOUND` | 404 | 资源不存在或对当前用户不可见 |
| `CONFLICT` | 409 | 唯一键冲突（含 Prisma `P2002`） |
| `VALIDATION_FAILED` | 400 | DTO 校验失败，`details` 给出字段级提示 |
| `UPSTREAM_UNAVAILABLE` | 502/503/504 | 第三方（ASR / LLM）不可用 |
| `INTERNAL_ERROR` | 500 | 其它未预期错误，不回传内部细节 |
