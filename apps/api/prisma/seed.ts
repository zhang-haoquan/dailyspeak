 ...
 * 种子内容库（PRD 07 章）
 *
 * 首发「计算机/IT + 职场通用」两个领域，共 10 张。
 * 这批内容原本硬编码在前端 `apps/web/src/data/cards.ts`，
 * Phase 1 迁移入库后前端不再持有任何内容数据。
 *
 * 后续由 P3 的内容管道（AI 生成候选 + 人工抽审）扩充。
 */
import { PrismaPg } from '@prisma/adapter-pg'
import { CardStatus, PrismaClient } from '../src/generated/prisma/client'
...
const CARDS: SeedCard[] = [
  {
    ...
    prompt: 'Could you describe a project you are most proud of and what you contributed?',
  ...
  },
  {
    ...
    sceneZh: '项目经验',
    sentence: 'I led the refactoring of our payment module, which cut processing time by 40%.',
  ...
  },
  {
    ...
    prompt: 'What programming languages or frameworks are you most comfortable with?',
  ...
  },
  {
    ...
    prompt: 'What did you work on yesterday and what will you do today?',
  ...
  },
  {
    ...
    sceneZh: '求职动机',
    sentence: 'Why are you interested in this position?',
    translation: '你为什么对这个职位感兴趣？',
    prompt: 'What attracts you to our company and this role?',
  ...
  },
  {
    ...
    prompt: 'Tell me about a time you handled a difficult situation at work.',
  ...
  },
  {
    ...
    sceneZh: '职业规划',
    sentence: 'In five years, I hope to grow into a technical lead and mentor junior engineers.',
  ...
  },
  {
    ...
    sceneZh: '邮件沟通',
    sentence: 'Could we move our meeting to 3 PM? I have a conflict at 2.',
    translation: '我们能把会议改到下午三点吗？两点我有冲突。',
    prompt: 'How would you politely reschedule a meeting with a colleague?',
  ...
  },
  {
    ...
    prompt: 'How do you respond when asked about your salary expectation?',
...
async function main(): Promise<void> {
  console.log(`[seed] 写入 ${CARDS.length} 张场景卡…`)

  for (const card of CARDS) {
    const data = { ...card, status: CardStatus.published }
    await prisma.scenarioCard.upsert({
      where: { id: card.id },
      create: data,
      update: data,
    })
  ...
  const byDomain = await prisma.scenarioCard.groupBy({
    by: ['domain'],
    _count: { _all: true },
  })
  console.log('[seed] 完成，当前内容库：')
  for (const row of byDomain) {
    console.log(`  ${row.domain}: ${row._count._all} 张`)
  }
}
